/// Top ports for network scanning, based on nmap frequency data
/// These are ordered roughly by frequency of open ports found in internet scans

/// Top 10 most commonly open ports - ultra-fast scan
pub const TOP_10_PORTS: &[u16] = &[80, 443, 22, 21, 25, 53, 110, 993, 995, 143];

/// Top 50 ports - quick scan with good coverage
pub const TOP_50_PORTS: &[u16] = &[
    21, 22, 23, 25, 53, 80, 110, 111, 113, 135, 139, 143, 199, 443, 445, 515, 554, 631, 993, 995,
    1080, 1433, 1434, 1723, 1900, 3306, 3389, 4444, 5000, 5432, 5566, 5631, 7001, 8000, 8001, 8008,
    8009, 8080, 8081, 8180, 8443, 8888, 9001, 9100, 9999, 49154, 81, 888, 1000, 2000,
];

/// Top 100 ports - balanced speed vs coverage
...
/// Top 1000 ports - comprehensive scanning
...
/// Quick scan ports - essential services only
pub const QUICK_SCAN_PORTS: &[u16] = &[
    21, 22, 23, 25, 53, 80, 110, 111, 135, 139, 143, 443, 445, 993, 995, 1433, 3306, 3389, 5432,
    8080, 8443,
];

/// Get default port list for scanning (100 ports for balanced speed/coverage)
pub fn get_default_ports() -> Vec<u16> {
    TOP_1000_PORTS.to_vec()
}

/// Get quick scan ports for fast reconnaissance (21 ports)
pub fn get_quick_ports() -> Vec<u16> {
    QUICK_SCAN_PORTS.to_vec()
}

/// Get top 10 ports for ultra-fast scanning
pub fn get_top_10_ports() -> Vec<u16> {
    TOP_10_PORTS.to_vec()
}

/// Get top 50 ports for quick but thorough scanning
pub fn get_top_50_ports() -> Vec<u16> {
    TOP_50_PORTS.to_vec()
}

/// Get top 100 ports for balanced speed vs coverage
pub fn get_top_100_ports() -> Vec<u16> {
    TOP_100_PORTS.to_vec()
}

/// Get top 1000 ports for comprehensive scanning
...
mod tests {
    ...
    #[test]
    fn test_port_list_lengths() {
        assert_eq!(
            TOP_10_PORTS.len(),
            10,
            "TOP_10_PORTS should have exactly 10 ports"
        );
        assert_eq!(
            TOP_50_PORTS.len(),
            50,
            "TOP_50_PORTS should have exactly 50 ports"
        );
        assert_eq!(
            TOP_100_PORTS.len(),
            100,
            "TOP_100_PORTS should have exactly 100 ports"
        );
        assert_eq!(
            TOP_1000_PORTS.len(),
            1000,
            "TOP_1000_PORTS should have exactly 1000 ports"
        );
        assert_eq!(
            QUICK_SCAN_PORTS.len(),
            21,
            "QUICK_SCAN_PORTS should have 21 ports"
        );
    }
    ...
    fn test_port_uniqueness() {
        ...
        let top_1000: HashSet<_> = TOP_1000_PORTS.iter().collect();
        assert_eq!(
            top_1000.len(),
            TOP_1000_PORTS.len(),
            "TOP_1000_PORTS contains duplicates"
        );

        let top_100: HashSet<_> = TOP_100_PORTS.iter().collect();
        assert_eq!(
            top_100.len(),
            TOP_100_PORTS.len(),
            "TOP_100_PORTS contains duplicates"
        );

        let top_50: HashSet<_> = TOP_50_PORTS.iter().collect();
        assert_eq!(
            top_50.len(),
            TOP_50_PORTS.len(),
            "TOP_50_PORTS contains duplicates"
        );

        let top_10: HashSet<_> = TOP_10_PORTS.iter().collect();
        assert_eq!(
            top_10.len(),
            TOP_10_PORTS.len(),
            "TOP_10_PORTS contains duplicates"
        );
    }

    #[test]
    fn test_essential_ports_included() {
        // Check that essential ports are included in all lists
        let essential_ports = [21, 22, 25, 53, 80, 443];

        for &port in &essential_ports {
            assert!(
                TOP_10_PORTS.contains(&port) || [80, 443, 22].contains(&port),
                "Essential port {} missing from appropriate lists",
                port
            );
            assert!(
                TOP_50_PORTS.contains(&port),
                "Essential port {} missing from TOP_50_PORTS",
                port
            );
            assert!(
                TOP_100_PORTS.contains(&port),
                "Essential port {} missing from TOP_100_PORTS",
                port
            );
            assert!(
                TOP_1000_PORTS.contains(&port),
                "Essential port {} missing from TOP_1000_PORTS",
                port
            );
        }
    }
